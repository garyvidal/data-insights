...
declare function a:analyze-json($objects as node()*) {
  ...
  let $obj-key := fn:string(fn:name($object))
  return
    typeswitch($object) 
     ...
     case object-node() | array-node() return
        ...
        let $propkey  := fn:concat($obj-key,$ELEMENT_CHILD_JOINER,$propname)
        let $xpath := xdmp:path($prop)
        let $_ := $ELEMENT_CHILD_FREQUENCY=>map:put($propkey, ($ELEMENT_CHILD_FREQUENCY=>map:get($propkey),1)[1] + 1)
        ...
        return
          ...
          then a:analyze-json($prop)
          else (::Terminal::)
            let $_ := $ELEMENT_CHILD_VALUES=>map:put($propkey, ($ELEMENT_CHILD_VALUES=>map:get($propkey), $prop))
            return ()
      default return ()
...
declare function a:analyze-root(
    ...
    let $stmt :=  
        if(fn:string($root/namespace) eq "") then
            fn:concat(
                "cts:search(/",
                $root/localname,",",
                xdmp:describe($query,(),()),
